...
export default function Example() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await axios.get('/api/BD/product');
            ...
            <a key={product._id} href={product.href || '#'} className="group">
              <div className="aspect-h-1 aspect-w-1 w-full overflow-hidden rounded-lg bg-gray-200 xl:aspect-h-8 xl:aspect-w-7">
                <img
                  ...
                  className="h-full w-full object-cover object-center group-hover:opacity-75 transition-transform transform group-hover:scale-110"
                />
              </div>
              <h3 className="mt-4 text-sm text-gray-700">{product.name}</h3>
